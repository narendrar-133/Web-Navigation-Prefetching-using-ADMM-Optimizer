...
def build_simple_lstm_model(item_vocab_size, seq_length):
    """Builds a single-layer LSTM model."""
    print("Building Simple, Single-Layer LSTM Model...")
    ...
    lstm_layer = LSTM(128)(emb_seq)
    output = Dense(item_vocab_size, activation='softmax')(lstm_layer)
    
    model = Model(inputs=input_seq, outputs=output)
    model.compile(optimizer='adam', loss='sparse_categorical_crossentropy', metrics=['accuracy'])
    print("Simple LSTM model built successfully.")
    model.summary()
    return model
...
def main():
    ...
    model = build_simple_lstm_model(item_vocab_size, SEQ_LENGTH)
